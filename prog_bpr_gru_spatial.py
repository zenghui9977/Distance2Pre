...
import os
import random
from utils import load_data, read_from_pkl, compute_distance
from BPR import OboBpr
from GRU import OboGru
from GRU_Spatial import OboDist2PreLinear, OboDist2PreNonLinear, softmax
from Global_Best import GlobalBest
from Load_Data_by_length import fun_data_buys_masks, cal_dis
from Load_Data_by_length import fun_random_neg_masks_tra, fun_random_neg_masks_tes, fun_compute_dist_neg
from Load_Data_by_length import fun_compute_distance, fun_acquire_prob
from Valuate import fun_predict_auc_recall_map_ndcg, fun_save_best_and_losses, fun_predict_auc_recall_map_ndcg_dist
...
class Params(object):
    def __init__(self, p=None):
        ...
        obfuscated_pois = read_from_pkl(p['process_data_folder'] + 'auxiliary_domain/', 'obfuscated_pois')
        confidence_matrix = read_from_pkl(p['process_data_folder'] + 'auxiliary_domain/', 'confidence_matrix')

        # 辅助域训练数据为加噪后的数据，测试数据保留原数据
        aux_tra_buys = obfuscated_pois
        aux_tra_dist = compute_distance(aux_tra_buys, pois_cordis, p['dd'], dist_num)
        ...
        tes_buys_masks, tes_masks = fun_data_buys_masks(tes_buys, tail=[item_num])
...
def train_valid_or_test():
    ...
    for epoch in np.arange(p['epochs']):
        ...
        if 0 == p['gru']:
            ...
            for uidx in user_idxs_tra:
                # TODO 修改处
                # uidx > aux_tra_num则表示属于目标域的用户节点，否则使用辅助域的训练函数
                if uidx > aux_tra_num:
                    loss += model.train(uidx)
                else:
                    loss += model.train_with_auxilary_data(uidx)
